...
- **Decoder:** VP8L (lossless) decoder with fault-tolerant mode (enabled by
  default) for pixel-level error recovery. Fills remaining pixels with gray when
  Huffman/LZ77 decoding fails.
...
- **Notes:** implemented: canonical Huffman generation, code-length RLE, and
  ...
  Fault-tolerant decoding allows partial recovery from corrupted VP8L images.
