...
export class AVIFFormat implements ImageFormat {
  ...
  async encode(imageData: ImageData): Promise<Uint8Array> {
    ...
    throw new Error(
      "AVIF encoding requires OffscreenCanvas API (not available in this runtime)",
    );
  }
  ...
  private async decodeUsingRuntime(
    ...
    throw new Error(
      "AVIF decoding requires ImageDecoder API (not available in this runtime)",
    );
  }

  /**
   * Parse EXIF metadata from AVIF data
   * AVIF files can contain EXIF data in the meta box
   * @param data Raw AVIF data
   * @param metadata Metadata object to populate
   */
  private parseEXIF(data: Uint8Array, metadata: ImageMetadata): void {
    // AVIF EXIF parsing would require navigating the ISOBMFF box structure
